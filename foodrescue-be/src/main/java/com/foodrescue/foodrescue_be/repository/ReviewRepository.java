...
public interface ReviewRepository extends JpaRepository<Review, Long> {
        ...
        Optional<Review> findByProductIdAndUserIdWithUser(@Param("productId") Long productId,
        ...
        @Query(value = "SELECT p.id as id, p.name as name, " +
                        ...
                        "COALESCE(COUNT(DISTINCT r.id), 0) DESC", nativeQuery = true)
        ...
        Page<Review> findSellerProductReviews(@Param("productId") Long productId, @Param("userId") Long userId,
                        Pageable pageable);
        ...
        @Query(value = """
                        ...
                        GROUP BY p.id, p.name
                        ORDER BY COALESCE(AVG(r.rating), 0) DESC,
                                 COALESCE(COUNT(DISTINCT r.id), 0) DESC
                        """, nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(DISTINCT p.id) FROM products p " +
                        "WHERE p.seller_id = (SELECT s.id FROM sellers s WHERE s.user_id = :userId)", nativeQuery = true)
